...
$abandoned_shops = mysqli_real_escape_string($con, $_GET['abandoned_shops']);
...
if($abandoned_shops != 0 AND $abandoned_shops != ''){
	switch ($abandoned_shops) {
      case "no_sale":
        $abandoned_shops_query=" AND S.shop_id NOT IN (SELECT  shop_id FROM pr_orders)";
        break;
      case "no_visit":
        $abandoned_shops_query=" AND S.shop_id NOT IN (SELECT shop_id FROM visits)";
        break;
      case "no_photo":
        $abandoned_shops_query=" AND S.shop_id NOT IN (SELECT shop_id FROM visit_images)";
        break;
      default:
        $abandoned_shops_query="";
    }
          ...
          <div class="col-12">
            ...
            <div class="card">
			  ...
			  <form action="/shops.php" method="GET">
				  ...
				  <div class="form-group col-md-2 ">
					<label for="login">Անտեսված </label>
					<select name="abandoned_shops" id="abandoned_shops"  class="form-control">
    					<option value="0"> Ընտրել </option>
    					<option <?php if($abandoned_shops === "no_sale") echo " selected"; ?> value="no_sale"> 0 վաճառք </option>
    					<option <?php if($abandoned_shops === "no_visit") echo " selected"; ?> value="no_visit"> 0 այցելություն </option>
    					<option <?php if($abandoned_shops === "no_photo") echo " selected"; ?> value="no_photo"> 0 նկար </option>
					</select>
				  </div>
