            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped" width="100%"> 
				 ...
				 <?php 
					...
					if($not_photo == 'on'){
						...
						$query = mysqli_query($con, "SELECT * FROM shops where shops.shop_id not in (SELECT shop_id FROM visit_images WHERE visit_images.date $query_date_range) $query_manager_1 $query_region_select $query_district_select $query_shop_select ");

					}else{
						$query = mysqli_query($con, "SELECT *, visit_id AS v_id FROM visit_images INNER JOIN visits ON visit_images.visit_id = visits.id RIGHT JOIN shops ON visits.shop_id = shops.shop_id RIGHT JOIN district on shops.district = district.id WHERE visits.date $query_date_range $query_manager_2 $query_region_select $query_district_select $query_shop_select GROUP by visit_images.visit_id ORDER by visits.date DESC");
				...
				if($not_photo == 'on'){
				 ...
				 <textarea id="visit_comment" data-visitid="<?php echo $visit_id; ?>" class="form-control visit_comment"><?php  echo $array_comment['comment']; ?></textarea>
